...
const GoldSilver = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE_URL}/gold_silver/analysis/`);
  ...
  if (loading) {
    return (
      <div className="gold-silver-container loading-container">
        ...
        <h2>Analyzing 1-Year Gold & Silver Data...</h2>
      </div>
    );
  }
  ...
  if (timeRange === '5D') {
    ...
    filteredTimeseries = data.timeseries.slice(-21); // Approx 21 trading days in a month
  } else if (timeRange === '1Y' || timeRange === 'Max') {
    // We fetched 1Y of data, so '1Y' and 'Max' are currently the full dataset length
    filteredTimeseries = data.timeseries;
  }

  // Generate ticks for the XAxis based on timeframe to keep it clean
  const getTickCount = () => {
    if (timeRange === '5D') return 5;
    if (timeRange === '1M') return 5;
    return undefined; // auto for 1Y/Max
  };
    ...
    <div className="gold-silver-wrapper">
      <div className="gs-header">
        ...
        <div className="current-prices">
          <span className="price-tag gold">10g 24k Gold: ₹{formatPrice(data.timeseries[data.timeseries.length - 1].gold_price)}</span>
          <span className="price-tag silver">1kg Silver: ₹{formatPrice(data.timeseries[data.timeseries.length - 1].silver_price)}</span>
        </div>
      </div>
          ...
          <div className="chart-container">
            <ResponsiveContainer width="100%" height={350}>
              <AreaChart data={filteredTimeseries} margin={{ top: 20, right: 0, left: 10, bottom: 5 }}>
                ...
                <XAxis dataKey="date" stroke="#718096" tick={{ fill: '#718096', fontSize: 12 }} tickCount={getTickCount()} minTickGap={30} tickLine={false} axisLine={false} />
